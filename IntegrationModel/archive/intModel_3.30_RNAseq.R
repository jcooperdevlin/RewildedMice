...
png("genes_cyt_corr_heat.png", height =5, width = 10, units = 'in', res=150)
...
pdf("total_loss_Genes.pdf", height = 10, width = 10)
gloss
dev.off()

write.table(results_df, "loss_results_Genes.txt", row.names=F, sep='\t', quote=F)
...
pdf("final_corr_Genes.pdf", height = 6, width = 14)
g1
dev.off()

write.table(cor_res, "final_corr_table_Genes.txt", sep='\t', quote=F, row.names=F)
save(modelList, file="modelList_Genes.RData")
